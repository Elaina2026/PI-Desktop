...
export function createSessionLaunchRuntime({
  ...
  async function resolveAgentRuntimeLaunch(
    ...
    async function ensureSessionProject(sid: string): Promise<string | null> {
      const id = sid.trim();
      if (!id) return getWorkspacePath();
      try {
        const res = (await runtimeState.host?.call("session.get", { id })) as
          | { session: { projectPath?: string } | null }
          | undefined;
        const existing = res?.session?.projectPath?.trim();
        if (existing) {
          sessionProjects.set(id, existing);
          return existing;
        }
        const activeWs = getWorkspacePath();
        if (activeWs) {
          await runtimeState.host?.call("session.update", { id, projectPath: activeWs }).catch(() => undefined);
          sessionProjects.set(id, activeWs);
          return activeWs;
        }
      } catch {}
      return sessionProjects.get(id) ?? getWorkspacePath();
    }
