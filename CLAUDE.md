...
## PI-Desktop Project Custom Rules & Architecture
...
### Performance Notes
- Token Usages (`statsGetModelUsageSummary`): Uses mtime+size cache (`sessionUsageScanCache` Map in main process) to skip unchanged files; pre-filters JSONL lines with `'"usage"'` substring before JSON.parse; async reads + `setImmediate` yield between files.
