...
- **排序**优惠最近更新 (`recent`)、创建日期 (`created`)、
  ...
  至 `recent`。为了兼容性，接受持久的 `manual` 值，但是
  此基线中不承诺任何拖动或手动重新排序交互。
- 每个项目组显示活动排序顺序中最新的十行
...
- 10px 左边缘分隔符锚定到按下位置并开始
  width，然后跟随指针 delta 而不跳转。
- 承诺的宽度限制在固定的 `244px–720px` 范围内。
- 指针移动是帧合并的。指针释放持续一提交
...
- 展开侧边栏的右边缘 `separator` 手柄以按下位置和起始宽度为锚点，预览
  240px 到 520px 之间的限制宽度，同时主窗格连续回流。
- 指针释放会提交并持久化最终宽度。指针取消、捕获丢失、组件卸载和
  Escape 会恢复按下时的宽度，不改变已保存的偏好。
- 聚焦手柄后，左右方向键每次调整 16px，Home 和 End 选择边界；键盘调整
  会立即提交，并暴露 `aria-valuenow` 与本地化宽度描述。
- 折叠侧边栏会隐藏手柄，但不会丢弃展开宽度；重新展开时恢复该宽度。

以下手势仍为未来的里程碑保留：

- 拖动 project/session 项目来分配手动订单
- 文件拖入输入框仍未处理；剪贴板 file/image 粘贴
  使用下面的会话临时参考流程

### 8. 2 规格保留

实施 drag/drop 时，应应用以下模式：

- 拖动手柄在悬停时必须可见（没有不可见的拖动可供性）
- 在悬停期间放置带有重音边框的目标突出显示
- 使用 Esc 键取消拖动
...
- 对于超过阈值的纯文本粘贴，渲染器通过相同的会话桥发送准确的 UTF-8
  `text/plain` 字节，在原始选择处插入生成的 `@<temporary-name>` 和空格，并在草稿中
  保存标记到规范路径的映射。标记保持在文本区域内而不是变成芯片；发送时在原位置
  只解析一次为规范临时路径。在草稿中间粘贴时，前缀和后缀都保持不变。
...
- Enter / Tab 接受突出显示的项目；菜单时 Enter 永远不会发送
  有一个突出显示的项目（该项目位于输入发送设置之前，该设置
  否则保持其行为）。
...
16. Drag/manual重排序未实现； `manual` 仍保持兼容性
    价值和未来阻力模式遵循§8
