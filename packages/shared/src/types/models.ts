...
export type TimeframeKey = "today" | "sevenDays" | "thirtyDays" | "sixtyDays" | "allTime";
...
export type ModelUsageDailyItem = {
  date: string;
  timestamp: number;
  inputTokens: number;
  cacheTokens: number;
...
export type ModelUsageSummaryResult = {
  timeframes: Record<TimeframeKey, TimeframeUsageItem>;
  models: ModelUsageSummaryItem[];
  daily: ModelUsageDailyItem[];
};
